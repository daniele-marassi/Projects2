    ...
    public partial class MediaBox : Form
    {
        ...
        private void UpdateCatalogBtn_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (SourceParam item in Statics.Sources.Where(_ => common.GetSourceValue(_.ValueStringJson).Type == sourceType.Directory))
                {
                    if (!Directory.Exists(common.GetSourceValue(item.ValueStringJson).Path)) { continue; }
                    listCatalog = multimediaCatalog.GetMedia(common.GetSourceValue(item.ValueStringJson).Path, this);
                ...
                {
                    if (!Directory.Exists(common.GetSourceValue(item.ValueStringJson).Path)) { continue; }
                    media_tmp = multimediaCatalog.PopolateCatalogFromSource(common.GetSourceValue(item.ValueStringJson).Path, this, nPictures);
        ...
        private void SetFonts()
        {
            ...
            this.SourceDelBtn.Font = utility.GetFonts("Wingdings 2", 16F, Statics.Fonts);
            this.SourceAddBtn.Font = utility.GetFonts("Web Symbols", 14.25F, Statics.Fonts);
            this.OpenCloseManageSourceBtn.Font = utility.GetFonts("Web Symbols", 18F, Statics.Fonts);
            ...
            this.DirectoryBtn.Font = utility.GetFonts("multimedia icons", 16F, Statics.Fonts);
            this.DriveBtn.Font = utility.GetFonts("Icons South St", 12F, Statics.Fonts);
        }
        ...
        private void SetToolTips()
        {
            ...
            toolTips.SetToolTip(OpenCloseManageSourceBtn, common.GetTraslation(OpenCloseManageSourceBtn.Name));            
        ...
        public void GetConfiguration()
        {
            ...
            sql = "SELECT ValueStringJson FROM Source;";
            DataTable source = sqLite.ExecuteSelect(sql);

            int index = 0; 

            foreach (DataRow row in source.Rows)
            {
                Statics.Sources.Add( new SourceParam { Index = index, ValueStringJson = row["ValueStringJson"].ToString() });
                index++;
            }
            ...
            foreach (Control ControlLevel0 in this.Controls)
            {
                ...
                foreach (Control ControlLevel1 in ControlLevel0.Controls)
                {
                    ...
                    foreach (Control ControlLevel2 in ControlLevel1.Controls)
                    {
                        ...
                        foreach (Control ControlLevel3 in ControlLevel2.Controls)
                        {
                            ...
                            foreach (Control ControlLevel4 in ControlLevel3.Controls)
                            {
                                ...
                                foreach (Control ControlLevel5 in ControlLevel4.Controls)
                                {
                                    ...
                                    catch (Exception ex) { Console.WriteLine(ex.ToString()); ControlLevel5.BackColor = utility.ConvertARGBtoRGB(Statics.FormParam.BackGrondColorControl); }
                                ...
                                catch (Exception ex) { Console.WriteLine(ex.ToString()); ControlLevel4.BackColor = utility.ConvertARGBtoRGB(Statics.FormParam.BackGrondColorControl); }
                            ...
                            catch (Exception ex) { Console.WriteLine(ex.ToString()); ControlLevel3.BackColor = utility.ConvertARGBtoRGB(Statics.FormParam.BackGrondColorControl); }
                        ...
                        catch (Exception ex) { Console.WriteLine(ex.ToString()); ControlLevel2.BackColor = utility.ConvertARGBtoRGB(Statics.FormParam.BackGrondColorControl); }
                    ...
                    catch (Exception ex){ Console.WriteLine(ex.ToString()); ControlLevel1.BackColor = utility.ConvertARGBtoRGB(Statics.FormParam.BackGrondColorControl); }
        ...
        private void OpenCloseManageSourceBtn_Click(object sender, EventArgs e)
        {
            OpenCloseManageSource();
        }

        private void OpenCloseManageSource()
        {
            SourceType sourceType = new SourceType();
            Common common = new Common();
            AccountDriveTxt.Text = common.GetTraslation(AccountDriveTxt.Name);
            PathAuthenticateJsonToDriveTxt.Text = common.GetTraslation(PathAuthenticateJsonToDriveTxt.Name);
            FolderNameToFilterDriveTxt.Text = common.GetTraslation(FolderNameToFilterDriveTxt.Name);
            if (ManageSourcesPnl.Height == 0)
            {
                SourceList.Items.Clear();
                foreach (SourceParam item in Statics.Sources)
                {
                    var source = common.GetSourceToList(item.ValueStringJson);

                    SourceList.Items.Add(source);
                }

                OpenMenuPanel(ManageSourcesPnl);
            }
            else { CloseMenuPanel(ManageSourcesPnl); }
        }

        private void SourceAddBtn_Click(object sender, EventArgs e)
        {
            SourceType sourceType = new SourceType();
            Common common = new Common();

            string sourceTypeSelected = String.Empty;

            if (Statics.Presentation.Drive == true) sourceTypeSelected = sourceType.Drive;
            if (Statics.Presentation.Directory == true) sourceTypeSelected = sourceType.Directory;

            var sourceValue = new SourceValue
            {
                Type = sourceTypeSelected,
                Path = DirectoryTxt.Text,
                AccountDrive = AccountDriveTxt.Text,
                PathAuthenticateJsonToDrive = PathAuthenticateJsonToDriveTxt.Text,
                FolderNameToFilterDrive = FolderNameToFilterDriveTxt.Text
            };
            
            var valueStringJson = new JavaScriptSerializer().Serialize(sourceValue);

            Statics.Sources.Add(new SourceParam() { Index = Statics.Sources.Count(),  ValueStringJson = valueStringJson });
            UpdateSources();
        }
        ...
        private void UpdateSources()
        {
            ...
            foreach (SourceParam item in Statics.Sources)
            {
                sql = $"INSERT INTO `Source` (`ValueStringJson`) VALUES ('{item.ValueStringJson}');";
                sqLite.ExecuteCommand(sql);
        ...
        private void SourceDelBtn_Click(object sender, EventArgs e)
        {
            ListBox.SelectedObjectCollection selectedItems = new ListBox.SelectedObjectCollection(SourceList);
            selectedItems = SourceList.SelectedItems;
            if (SourceList.SelectedIndex != -1)
            {
                for (int i = selectedItems.Count - 1; i >= 0; i--)
                {
                    Statics.Sources.RemoveAt(SourceList.SelectedIndex);
                    SourceList.Items.Remove(selectedItems[i]);
                }
            }
            UpdateSources();
        }

        private void DeleteSources(string where = null)
        {
            SqLite sqLite = new SqLite();
            if (Statics.FormParam.Demo != DemoStatus.NotAvviable && Statics.FormParam.Demo != DemoStatus.ReStart)
            { sqLite.OpenDB(Statics.Constant.FilePathDemoDB); }
            ...
            string sql = $"DELETE FROM Source {where};";
            sqLite.ExecuteCommand(sql);
            sqLite.CloseDB();
        }
        ...
        private void RepositionButtonMenu()
        {
            ...
            top = 5;
            obj = OpenCloseManageSourceBtn;
            obj.Top = top;

            top = obj.Top + obj.Height + 0;
            obj = ManageSourcesPnl;
            obj.Top = top;
        ...
        public void DriveOrDirectory(string btnName)
        {
            Utility utility = new Utility();
            if (btnName == "DirectoryBtn")
            {
                DriveBtn.BackColor = Color.Transparent;

                DirectoryBtn.BackColor = utility.ConvertARGBtoRGB(Statics.FormParam.BackGrondColorControlSelected);  

                DirectoryOpenBtn.Enabled = true;
                DirectoryTxt.Enabled = true;
                Statics.Presentation.Directory = true;
                AccountDriveTxt.Enabled = false;
                PathAuthenticateJsonToDriveTxt.Enabled = false;
                FolderNameToFilterDriveTxt.Enabled = false;
                Statics.Presentation.Drive = false;
            }

            if (btnName == "DriveBtn")
            {
                DriveBtn.BackColor = utility.ConvertARGBtoRGB(Statics.FormParam.BackGrondColorControlSelected);

                DirectoryBtn.BackColor = Color.Transparent;

                DirectoryOpenBtn.Enabled = false;
                DirectoryTxt.Enabled = false;
                Statics.Presentation.Directory = false;
                AccountDriveTxt.Enabled = true;
                PathAuthenticateJsonToDriveTxt.Enabled = true;
                FolderNameToFilterDriveTxt.Enabled = true;
                Statics.Presentation.Drive = true;
            }
        }
    }
}
