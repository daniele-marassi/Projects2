    ...
    public static partial class HookManager
    {
        ...
        private static int KeyboardHookProc(int nCode, Int32 wParam, IntPtr lParam)
        {
            ...
            if (nCode >= 0)
            {
                ...
                //raise KeyDown
                if ((s_KeyDown != null || getKeyDown) && (wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN))
                {
                    Keys keyData = (Keys)MyKeyboardHookStruct.VirtualKeyCode;
                    KeyEventArgs e = new KeyEventArgs(keyData);
                    _key = keyData.ToString();
                    if (s_KeyDown != null)
                    {
                    ...
                    if (getKeyDown)
                    {
                        if (_project_Domain_Class_KeyDown != String.Empty && _methodName_KeyDown != String.Empty)
                            utility.CallMethodByName(_project_Domain_Class_KeyDown, _methodName_KeyDown, _key);
                    }
                }

                // raise KeyPress
                if ((s_KeyPress != null || getKeyPress) && wParam == WM_KEYDOWN)
                {
                              ...
                              MyKeyboardHookStruct.Flags) == 1)
                    {
                        ...
                        if (getKeyPress)
                        {
                            if (_project_Domain_Class_KeyPress != String.Empty && _methodName_KeyPress != String.Empty)
                                utility.CallMethodByName(_project_Domain_Class_KeyPress, _methodName_KeyPress, _key);
                        }
                    }
                }

                // raise KeyUp
                if ((s_KeyUp != null || getKeyUp) && (wParam == WM_KEYUP || wParam == WM_SYSKEYUP))
                {
                    Keys keyData = (Keys)MyKeyboardHookStruct.VirtualKeyCode;
                    KeyEventArgs e = new KeyEventArgs(keyData);
                    _key = keyData.ToString();
                    ...
                    if (getKeyUp)
                    {
                        if (_project_Domain_Class_KeyUp != String.Empty && _methodName_KeyUp != String.Empty)
                            utility.CallMethodByName(_project_Domain_Class_KeyUp, _methodName_KeyUp, _key);
                    }
                }
